...
label: SQLite
order: 70
desc: Payload는 공식적으로 지원되는 Drizzle 데이터베이스 어댑터를 통해 SQLite를 지원합니다.
keywords: SQLite, 문서, typescript, 콘텐츠 관리 시스템, cms, 헤드리스, javascript, node, react, nextjs
---

Payload를 SQLite와 함께 사용하려면 `@payloadcms/db-sqlite` 패키지를 설치하세요. 이는 Drizzle ORM과 `libSQL`을 활용하여 제공하는 SQLite 데이터베이스와 상호작용합니다.

개발 모드에서 데이터베이스에 대한 변경 사항을 자동으로 관리하고, 스키마와 다른 데이터베이스 환경을 동기화하는 데 활용할 수 있는 완전한 마이그레이션 제어 도구를 제공합니다. DDL 변환은 자동으로 생성됩니다.

Payload가 SQLite를 사용하도록 구성하려면 다음과 같이 Payload Config에 `sqliteAdapter`를 전달하세요:
...
export default buildConfig({
  ...
  // 여기서 SQLite 어댑터를 구성합니다
  db: sqliteAdapter({
    // SQLite 관련 인수가 여기에 들어갑니다.
    // `client.url`은 필수입니다.
    client: {
...
| 옵션                       | 설명                                                                                                                                                                            |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client` \*                | `@libsql/client`의 `createClient`에 전달될 [클라이언트 연결 옵션](https://orm.drizzle.team/docs/get-started-sqlite#turso).                                                      |
...
| `logger`                   | drizzle에 전달될 로거의 인스턴스. 기본적으로 Payload의 것이 사용됩니다.                                                                                                         |
| `idType`                   | id 컬럼에 주어지는 데이터 타입에 사용되는 'number' 또는 'uuid' 문자열입니다.                                                                                                   |
| `transactionOptions`       | 트랜잭션을 위한 SQLiteTransactionConfig 객체 또는 트랜잭션 사용을 비활성화하려면 `false`로 설정합니다. [자세한 내용](https://orm.drizzle.team/docs/transactions)                |
...
| `autoIncrement`            | 삭제된 행에서 동일한 ID가 재사용될 수 없도록 보장하기 위해 기본 키에 대한 SQLite [AUTOINCREMENT](https://www.sqlite.org/autoinc.html)를 활성화하려면 `true`를 전달합니다         |
...
// Drizzle을 설치하지 않으려면 재내보내기 경로에서 drizzle이 가진 모든 것을 가져올 수 있습니다.
...
Drizzle을 직접 노출하는 것 외에도 모든 테이블과 Drizzle 관계도 `payload.db` 속성을 통해 노출됩니다.
...
SQLite에서 마이그레이션은 Payload를 사용하는 데 있어 기본적인 측면이며 작동 방식에 익숙해져야 합니다.
