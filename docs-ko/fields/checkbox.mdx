...
label: 체크박스
order: 40
desc: 체크박스 필드 타입은 개발자가 데이터베이스에 불린 값을 저장할 수 있게 해줍니다. 체크박스 필드 사용법, 예제 및 옵션을 알아보세요.
keywords: 체크박스, 필드, 구성, 설정, 문서, 콘텐츠 관리 시스템, cms, 헤드리스, javascript, node, react, nextjs
---

체크박스 필드는 데이터베이스에 불린(boolean) 값을 저장합니다.
...
<LightDarkImage
  ...
  alt="Payload 관리자 패널의 텍스트 필드가 있는 체크박스 필드"
  caption="아래에 텍스트 필드가 있는 체크박스 필드의 관리자 패널 스크린샷"
/>

체크박스 필드를 추가하려면 [필드 Config](./overview)에서 `type`을 `checkbox`로 설정하세요:
...
| **`defaultValue`**     | 이 필드의 기본값으로 사용할 데이터를 제공합니다. 필드가 `required`인 경우 기본값은 false입니다. [자세한 내용](/docs/fields/overview#default-values).                    |
...
export const ExampleCollection: CollectionConfig = {
  ...
  fields: [
    {
      name: 'enableCoolStuff', // 필수
      type: 'checkbox', // 필수
      label: '멋진 기능을 보려면 클릭하세요',
      defaultValue: false,
    },
  ],
}
